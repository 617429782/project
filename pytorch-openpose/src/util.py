...
def padRightDownCorner(img, stride, padValue):
    ...
    img_padded = img
    pad_up = np.tile(img_padded[0:1, :, :]*0 + padValue, (pad[0], 1, 1))    # np.tile()将原矩阵横向、纵向地复制
    img_padded = np.concatenate((pad_up, img_padded), axis=0)               # np.concatenate()数组的拼接
...
# 绘制accuracy_loss
def draw_accuracy_loss(loss_list, accuracy_list, epo_num, stage, time, idx):
    """
    :param loss_list:
    :param accuracy_list:
    :param epo_num:
